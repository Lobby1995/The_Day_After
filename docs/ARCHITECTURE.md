...
The play screen has a main column (the place in the world view, then the question) and a side column that reads top to bottom: the survivor's condition (`#vitals`), the report on the world, and the survivor's file (`#side`). In the world view the page never scrolls when a question opens; the place shrinks to a strip above it, centred on the survivor.

The condition block (`#vitals`) is kept short so the report on the world stays near the top of the side column: the year and season, the state and supplies, one line per meter (health, infection, morale), the medicine button, and then the marks, companions and journal folded into one strip (`<details id="vmore">`, closed by default; `VMORE` remembers whether it was left open).
